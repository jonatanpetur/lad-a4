...
/**
 * Created by samz on 3/11/16.
 */

public class Task2_SamiSindi<E extends Comparable<E>> implements SortedList<E> {
     ...
     * Average case complexity for this hash code is O(n/m) , n is the number of items and m is the size of the string.
     * @return
     */
    @Override
    public int hashCode(){
        int hashCode = 0;
        if(count > 0){
            for( int i = 0; i < count; i++) { // O(1)
                return 31*hashCode + this.get(i).hashCode();//O(1) assuming get(i) is O(1)
            }
        }
    ...
    /**
     *  Worst-case complexity is O(N) where N is the number of elements in the list. Assuming that
     *  equals() methods called within the method is O(1).
     *  Since there is a loop which executes N times and has constant time operations within, the total worst-case time
     *  complexity is O(N).
     *  The method compares two elements and return true if they have the same hash code or false if they don't
     * @return
     */
    @Override
    public boolean equals(Object o) {

        Task2_SamiSindi list = (Task2_SamiSindi) o;
        if (this.hashCode() == list.hashCode()) return true; //O(1)
        else {
            return false;
        }
    }
